...
# Test the model with LBA on demo data #

# -- (1) Run DSO to get the initial poses -- #
# parameters for run_dso.py: 
...
# parameters for test_KVNet_LBA_.py: 
...
# - dataset: the dataset name. Should one of scanNet, 7scenes, kitti, single_folder
#            For single folder, the dataset_path is the path of the folder that includes the input rgb images
# - dataset_path: the path to the specified dataset
# - model_path: the path to the trained model
# - img_name_pattern : used for the case where dataset == single_folder. The image name pattern, e.g. *.color.png for frame_#.color.png for 7scene and scanNet name pattern 
...
CUDA_VISIBLE_DEVICES=0 python3 test_KVNet_LBA.py \
 		--exp_name LBA_demo \
 		--sigma_soft_max 10 \
 		--t_win 2 \
 		--d_min .1  --d_max 5. \
        ...
        --use_dso_R  \
 		--opt_next_frame \
 		--min_frame_idx 0  --max_frame_idx 298 \
 		--refresh_frames 300 \
